...
7. **Scroll-driven arrival (added 2026-09-17).** A `scroll-reveal` utility: a block settles 18px into place over the first third of its entry into the viewport, transform only (a fade blended muted text below the 4.5:1 contrast floor while blocks waited below the fold; Lighthouse mobile caught it on production at 96 accessibility), on a CSS view timeline (no JS, no smooth-scroll library, native scrolling untouched). Applied to the six home chapters, the essays/notes/library index content, and Journey chapters. Static without support and under reduced motion.
8. **Journey as one thread (added 2026-09-17, after the owner's reference: a fixed identity block beside a single dated thread).** The sticky aside becomes the identity block (name, India / elsewhere, Est. Punjab, India's clock, the visible route: all strings the site already asserts, no coordinates invented). The numbered squares and per-chapter segments become one continuous thread that fills with reading progress (the existing Framer scroll value, now `scaleY`); each chapter is a 5px marker on the thread with its phase set in meta caps to the left and the chapter to the right; the marker grows into a 9px origin while its chapter passes the middle of the viewport (view timeline), on hover, focus, or as the `:target`. Phone: the phase returns to the meta row and the thread hugs the left edge.
...
### Files (Journey and scroll-reveal, 2026-09-17)

`components/journey-explorer.tsx` (identity block, single thread, phase column, markers), `app/globals.css` (Journey thread/marker/identity styles, `scroll-reveal` utility), `app/page.tsx` and the essays/notes/library pages (class only), `tests/e2e/v5-journey.spec.ts` (4 checks), `tests/e2e/site.spec.ts` (progress now vertical), 24 Journey baselines.
