...
7. **Scroll-driven arrival (added 2026-09-17).** A `scroll-reveal` utility: a block settles 14px into place from 30% opacity over the first third of its entry into the viewport, on a CSS view timeline (no JS, no smooth-scroll library, native scrolling untouched). Applied to the six home chapters, the essays/notes/library index content, and Journey chapters. Static without support and under reduced motion.
